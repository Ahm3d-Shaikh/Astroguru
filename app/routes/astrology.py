...
async def fetch_astrology_details(user_question_object: UserQuestionObj, profile_id: str = Query(None), language: str = Query("English"), user = Depends(get_current_user)):
    try:
        ...
        result, category, new_conversation_id, coins = await fetch_predictions_for_user(user_id, profile_id, user_question_object.user_question, conversation_id, language)
